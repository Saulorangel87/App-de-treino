...
BEGIN
    ...
    IF NEW.pain_reported THEN
        adaptation_kind := 'safety'; affected_sessions := 2; duration_factor := 0.80;
        target_rpe_cap := 3; minimum_target_rpe := 2;
    ...
    ELSIF actual_rpe >= 9 OR NEW.fatigue_after = 5 OR NEW.difficulty = 'very_hard' THEN
        adaptation_kind := 'recovery'; affected_sessions := 2; duration_factor := 0.80;
        target_rpe_delta := -1; target_rpe_cap := 4; minimum_target_rpe := 3;
    ...
    ELSIF actual_rpe >= source_target_rpe + 2 OR NEW.fatigue_after >= 4 OR NEW.difficulty = 'hard' THEN
        adaptation_kind := 'recovery'; affected_sessions := 1; duration_factor := 0.90;
        target_rpe_delta := -1; minimum_target_rpe := 3;
    ...
    ELSIF actual_rpe <= source_target_rpe - 2
        ...
        adaptation_kind := 'progression'; affected_sessions := 1; duration_factor := 1.05;
    ...
    LOOP
        ...
        new_explanation := jsonb_set(
            COALESCE(candidate.explanation, '{}'::jsonb), '{adaptation}',
            jsonb_strip_nulls(jsonb_build_object(
                'kind', adaptation_kind, 'reason', adaptation_reason,
                'safety_notice', safety_notice, 'source_workout_id', source_workout_id,
                'previous_duration_minutes', candidate.duration_minutes,
                'previous_target_rpe', candidate.target_rpe
            )), true);
        new_explanation := jsonb_set(
            new_explanation, '{rules}',
            COALESCE(new_explanation->'rules', '[]'::jsonb) || jsonb_build_array(adaptation_reason), true);
        UPDATE workouts
        SET duration_minutes = new_duration, target_rpe = new_target_rpe, explanation = new_explanation
