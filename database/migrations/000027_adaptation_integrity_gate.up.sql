...
BEGIN
    SELECT ws.workout_id, ws.athlete_profile_id, ws.actual_rpe, ws.duration_minutes,
           w.training_plan_id, w.scheduled_on, w.target_rpe
    INTO source_workout_id, source_profile_id, actual_rpe, actual_duration,
    ...
    -- Partial sessions and invalid minimum data are retained for audit but
    -- cannot change future prescription through the active trigger.
    IF NEW.completion_status <> 'complete'
       OR actual_duration IS NULL OR actual_duration <= 0
       OR actual_rpe IS NULL OR actual_rpe < 1 OR actual_rpe > 10
       OR NEW.fatigue_after IS NULL OR NEW.fatigue_after < 1 OR NEW.fatigue_after > 5 THEN
        RETURN NEW;
    END IF;

    -- A recent protective signal wins over a single neutral/easy response.
    IF EXISTS (
        SELECT 1
        FROM workout_sessions ws_recent
        JOIN workouts w_recent ON w_recent.id = ws_recent.workout_id
        JOIN feedback f_recent ON f_recent.workout_session_id = ws_recent.id
        WHERE w_recent.training_plan_id = source_plan_id
          AND w_recent.scheduled_on < source_date
          AND w_recent.scheduled_on >= source_date - 14
          AND ws_recent.status = 'completed'
          AND (f_recent.pain_reported = true
               OR f_recent.fatigue_after >= 4
               OR ws_recent.actual_rpe >= w_recent.target_rpe + 2)
    ) THEN
        RETURN NEW;
    END IF;
