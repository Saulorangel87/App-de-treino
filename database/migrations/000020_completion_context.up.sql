...
ALTER TABLE feedback
    ADD CONSTRAINT feedback_completion_context_check
    CHECK (
        completion_status IN ('complete', 'partial')
        AND (
            (completion_status = 'complete' AND partial_reason IS NULL)
            OR (
                completion_status = 'partial'
                AND partial_reason IS NOT NULL
                AND partial_reason IN (
...
BEGIN
    ...
    ELSIF NEW.completion_status = 'complete'
          AND actual_rpe <= source_target_rpe - 2
