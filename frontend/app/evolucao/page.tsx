...
function speedLabel(value: number) { return `${value.toFixed(1).replace('.', ',')} km/h`; }
function goalLabel(value: string) { return ({ health: 'Saúde', fitness: 'Condicionamento', endurance: 'Resistência', performance: 'Desempenho', event: 'Prova', weight_management: 'Gestão de peso' } as Record<string, string>)[value] || value; }
function readinessLabel(value: RecoveryPoint['readiness']) { return value === 'ready' ? 'Adequada' : value === 'caution' ? 'Atenção' : 'Recuperação'; }
function compactDate(value: Date) { return shortDate.format(value).replace(/\./g, '').replace(' de ', ' '); }
function weekRangeLabel(value: string) { const start = parseDate(value); const end = new Date(start); end.setDate(end.getDate() + 6); return `${compactDate(start)} – ${compactDate(end)}`; }
  ...
  return <main className="evolution-shell">
    ...
    <section className="evolution-content">
      ...
      {!hasActivities ? <section className="evolution-empty"><LineChart size={28} /><h2>Os primeiros dados aparecerão após seus treinos.</h2><p>Ao concluir ou cancelar sessões, o Cadência passará a organizar sua consistência, duração e esforço percebido aqui.</p><a href="/plano">Ver meu plano</a></section> : <>
        ...
        {goalProgress.length > 0 && <section className="evolution-card goal-progress-card"><div className="evolution-card-title"><div><span>ACOMPANHAMENTO DE OBJETIVOS</span><h2>O que foi registrado para sua direção atual</h2></div><small>Últimos 28 dias</small></div><p className="goal-progress-intro">O Cadência mostra os registros ligados ao seu contexto atual sem inventar uma porcentagem de conclusão para metas que não têm uma medida final definida.</p><div className="goal-progress-list">{goalProgress.map((goal) => <article key={`${goal.priority}-${goal.goal_type}`}><div><span>{goal.priority === 1 ? 'OBJETIVO PRINCIPAL' : 'OBJETIVO SECUNDÁRIO'}</span><strong>{goalLabel(goal.goal_type)}</strong>{goal.target_date && <small>Data-alvo: {shortDate.format(parseDate(goal.target_date)).replace('.', '')}</small>}</div><div className="goal-progress-metrics"><span><b>{goal.completed_sessions}</b> sessões</span><span><b>{minutesLabel(goal.total_minutes)}</b> pedalados</span>{goal.total_distance_km > 0 && <span><b>{distanceLabel(goal.total_distance_km)}</b> registrados</span>}{goal.session_rpe_load > 0 && <span><b>{Math.round(goal.session_rpe_load)}</b> carga RPE</span>}{goal.average_speed_kph > 0 && <span><b>{speedLabel(goal.average_speed_kph)}</b> média</span>}</div>{goal.details && <p>{goal.details}</p>}</article>)}</div></section>}
        ...
        {hasCyclingMetrics && <section className="evolution-card cycling-overview"><div className="evolution-card-title"><div><span>DADOS DO PEDAL</span><h2>Métricas registradas</h2></div></div><div className="cycling-overview-grid">{summary.total_distance_km > 0 && <Metric icon={<MapPinned size={18} />} value={distanceLabel(summary.total_distance_km)} label="distância acumulada" />}{summary.total_elevation_m > 0 && <Metric icon={<Mountain size={18} />} value={`${Math.round(summary.total_elevation_m)} m+`} label="elevação acumulada" />}{summary.average_power_watts > 0 && <Metric icon={<Zap size={18} />} value={`${Math.round(summary.average_power_watts)} W`} label="potência média registrada" />}{summary.average_heart_rate > 0 && <Metric icon={<HeartPulse size={18} />} value={`${Math.round(summary.average_heart_rate)} bpm`} label="frequência cardíaca média" />}{summary.average_speed_kph > 0 && <Metric icon={<Bike size={18} />} value={speedLabel(summary.average_speed_kph)} label="velocidade média registrada" />}{summary.session_rpe_load > 0 && <Metric icon={<Target size={18} />} value={String(Math.round(summary.session_rpe_load))} label="carga sessão-RPE" />}</div><p className="chart-caption">A carga sessão-RPE é duração em minutos × RPE realizado. Velocidade e métricas físicas aparecem apenas quando foram registradas; elas descrevem registros, não estimam desempenho.</p></section>}
      </>}
