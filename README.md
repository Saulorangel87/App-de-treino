...
Versão publicada: `0.12.0` — correções de segurança, sessões adaptadas iniciáveis e planejamento por proximidade do evento. Consulte o [release v0.12.0](https://github.com/Saulorangel87/App-de-treino/releases/tag/v0.12.0).
...
A tela `/plano` gera, apresenta e ativa ciclos de quatro semanas. O motor `rules-v1` é determinístico: considera experiência, objetivo, limitações, disponibilidade, o contexto opcional de ciclismo e um resumo observado dos últimos 28 dias de sessões e recuperação. Ele seleciona sessões específicas de forma gradual (cadência no indoor, subidas, sweet spot por potência/FTP, ritmo de prova e os pilotos de intervalos moderados de estrada, intensos de estrada, VO₂max de estrada e aeróbicos XCO), limita cada sessão ao tempo informado e reduz a intensidade quando há uma condição de segurança ativa ou sinais recentes de recuperação insuficiente. O piloto VO₂max exige preferência explícita, estrada, nível avançado, avaliação apta e histórico mínimo; foi validado no checkout local e permanece fora da produção até revisão, ampliação do catálogo e autorização de publicação. O dashboard usa o plano aprovado, explica a escala RPE e permite acompanhar a sessão do início ao feedback pós-treino. No desenvolvimento local, novos rascunhos também congelam no `prescription_snapshot` uma classificação observacional de prontidão e medições de 7/28/42 dias de aderência e carga por session-RPE; esses campos ainda não alteram a prescrição.
...
O checkout local também contém o piloto `taper-v1`, que reduz de forma conservadora o volume de sessões antes de um evento próximo quando os gates de elegibilidade e segurança são atendidos, e o piloto `road_vo2_intervals`, selecionado pela preferência explícita `vo2max` em estrada avançada elegível. A versão local está em `0.14.0`; os dois pilotos continuam fora da produção até revisão, ampliação do catálogo e autorização de publicação.
