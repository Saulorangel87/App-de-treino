...
- A produção permanece em `c768ef7`, com a API funcional de `5fbc668`. O checkout local está no commit `64e554d`, que registra a avaliação shadow do `rules-v2`; a matriz controlada e a conferência manual da API foram concluídas, e há apenas um teste regressivo adicional ainda sem commit nem deploy. O catálogo inicial e o piloto de estrada estão publicados, sujeitos aos critérios de elegibilidade documentados.
