...
- Produção implantada na VPS Oracle no commit `d1cc7e4`, conforme deploy informado pelo proprietário; as migrações `000017`–`000021` continuam aplicadas e a versão visível do produto permanece `0.20.0`. A release correspondente é [v0.20.0](https://github.com/Saulorangel87/App-de-treino/releases/tag/v0.20.0).
...
- A produção está no commit `d1cc7e4` e na versão `0.20.0`, com correções de segurança, catálogo ampliado, contexto de conclusão/feedback pós-treino e o gate observacional da distribuição dos estímulos publicados conforme os critérios documentados. A tela `/plano` carregou após o deploy; a leitura autenticada do JSON interno ainda precisa ser conferida por uma requisição da própria sessão do navegador.
