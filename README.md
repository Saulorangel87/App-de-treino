...
- A produção permanece em `c768ef7`, com a API funcional de `5fbc668`. O checkout local está no commit `810183c`, que registra o histórico observacional `training-history-v3`; a avaliação local em shadow do `rules-v2` ainda está sem commit nem deploy. O catálogo inicial e o piloto de estrada estão publicados, sujeitos aos critérios de elegibilidade documentados.
