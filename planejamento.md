...
Os cinco testes do novo avaliador passaram junto dos testes anteriores do shadow. Na sétima fatia, a avaliação é executada na mesma transação do feedback e gravada em `workouts.explanation.adaptation_shadow`; uma falha histórica é isolada por savepoint e registrada como `not_evaluated`. Os pacotes Go alterados, `go vet` e o build do frontend também passaram. Não houve migração, mudança visual, infraestrutura ou alteração das notas de versão. O próximo passo é conferir manualmente `adaptation_shadow` no `GET /v1/plans/current` depois de concluir um treino local, preservando o trigger existente até haver comparação e autorização explícita.
