...
Sobre o commit local `b0f32b7`, a quarta fatia foi implementada sem commit ou deploy. Novos rascunhos usam `training-history-v3` e registram `period_comparison` com seis períodos semanais não sobrepostos nos últimos 42 dias: `last_7d`, `days_8_14`, `days_15_21`, `days_22_28`, `days_29_35` e `days_36_42`. Cada período preserva as medições brutas de aderência, sessões realizadas, carga session-RPE, feedback, sinais protetivos e recuperação.
...
Os testes Go, `go vet`, build do frontend, validação estrutural do OpenAPI e a consulta PostgreSQL com fixtures sintéticas em transação somente leitura passaram. Nenhum dado real foi lido ou alterado. Não houve mudança visual, migração, infraestrutura ou prescrição; `APP_VERSION` e `UPDATE_NOTES` continuam em `0.7.0`. Falta apenas gerar um novo rascunho local e conferir no `GET /v1/plans/current` a versão v3, os seis períodos e a invariância dos treinos antes de propor o commit.
