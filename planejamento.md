...
A validação pós-deploy confirmou os serviços saudáveis, `/health` e `/ready` internos, HTTP 200 nos dois domínios oficiais, a presença da versão `0.20.0` no HTML público e HTTP 401 para `/v1/plans/current` sem sessão. A release [v0.20.0](https://github.com/Saulorangel87/App-de-treino/releases/tag/v0.20.0) foi publicada no GitHub. O `rules-v1` continua prescritivo; as avaliações shadow, a comparação planejado versus realizado e os contextos pós-treino continuam observacionais.

### Continuidade — distribuição observacional dos estímulos — 13 de setembro de 2026

A próxima fatia foi implementada localmente para atender aos tópicos de carga/progressão, periodização e seleção de estímulos sem transferir autoridade ao `rules-v2`. O histórico agora usa `training-history-v4`, o comparador passa a `period-comparison-v2` e cada período semanal registra sessões de qualidade (`target_rpe >= 6,0`), sessões de alta intensidade (`target_rpe >= 7,0`), minutos realizados, cobertura de session-RPE e densidade relativa às sessões realizadas.

O novo bloco `stimulus-distribution-v1` resume 7, 14, 28 e 42 dias e registra pares de sessões em dias consecutivos, menor intervalo e recência do estímulo. A proximidade usa o dia UTC de `completed_at`, pois ainda não há fuso individual; sessões futuras e dados inelegíveis pelo `data-integrity-v1` ficam fora. Os cortes são convenções de agrupamento do produto, não zonas fisiológicas, diagnóstico ou limiar universal de risco.

Esta entrega é somente observacional e backend/documental: `rules-v1` continua prescritivo, `progression_eligible`, `applied` e `used_for_prescription` continuam falsos, e nenhum treino é alterado. Não há migração, nota de versão, mudança visual, deploy ou infraestrutura. `go test -count=1 ./...`, `go vet ./...`, `npm run build`, lint direcionado, a fixture PostgreSQL somente leitura, validação de referências do OpenAPI e `git diff --check` passaram. O lint geral ainda possui débitos preexistentes fora dos arquivos tocados.

### Continuidade — gate observacional da distribuição dos estímulos — 13 de setembro de 2026

O `rules-v2` e o `rules-v2-adaptation-v1` agora consomem a medição `stimulus-distribution-v1` em modo shadow. O gate identifica, de forma determinística, pelo menos duas sessões de qualidade em sete dias ou sessões de qualidade em dias consecutivos nos 42 dias observados. Com cobertura válida, a resposta candidata é protetiva e não progressível; com lacunas ou inconsistências, a progressão permanece não avaliada/adiada. Nenhum desses resultados altera a prescrição.

O resultado inclui a distribuição observada, o `rules_evaluated` registra `stimulus_distribution_gate` e o `decision_audit` consolida o dado e a restrição quando aplicável. Ausência de sessões de qualidade não é bloqueio quando o histórico está íntegro. `rules-v1` continua prescritivo, todos os campos de autoridade permanecem falsos e não houve migração, mudança visual, release, deploy ou infraestrutura.

Foram adicionados testes para densidade, proximidade, cobertura de datas, ausência de sessões de qualidade, auditoria e invariância de não autoridade. `go test -count=1 ./...`, `go vet ./...`, `npm run build`, lint direcionado e `git diff --check` passaram. O commit `d1cc7e4` foi implantado pelo proprietário; não houve mudança de versão visível, migração ou alteração de infraestrutura. A tela autenticada `/plano` carregou em produção, mas a leitura direta da rota API foi bloqueada no navegador e não houve inspeção independente do JSON nesta sessão. Próxima etapa: revisar os resultados shadow em cenários reais controlados ou por uma requisição autenticada do próprio fluxo e só depois avaliar a coerência com periodização e seleção de estímulo, sem ativar prescrição automática.

### Continuidade — auditoria observacional da periodização — 13 de setembro de 2026

Depois da validação controlada do gate de distribuição, a próxima fatia foi implementada localmente como `periodization-shadow-v1`. Na geração de cada rascunho, o bloco observa as quatro semanas planejadas, classifica as três primeiras como `progression` e a quarta como `recovery`, e registra sessões, minutos, qualidade, alta intensidade, recuperação, endurance longo, taper e espaçamento entre estímulos exigentes.

O auditor verifica a presença das semanas, a ausência de qualidade na semana de recuperação, a redução do volume da recuperação, o limite de uma sessão de qualidade por semana e a ausência de sessões de qualidade em dias consecutivos. Dados ausentes ficam em `missing_data` e incoerências em `data_issues`; a ausência de sessões de qualidade não é tratada como erro. A avaliação permanece `mode: shadow`, com `progression_eligible: false`, `applied: false` e `used_for_prescription: false`, sem escolher ou alterar qualquer treino do `rules-v1`.

Foram adicionados testes para ciclo coerente, semanas ausentes, qualidade na semana de recuperação e invariância do motor prescritivo. `go test -count=1 ./...`, `go vet ./...`, `npm run build`, lint direcionado, validação das referências do OpenAPI e `git diff --check` passaram. O commit `b32a3c9` foi registrado e publicado após autorização, sem migração, mudança visual ou atualização da versão. A versão visível permanece `0.20.0`.
...
A tela de acompanhamento passa a avisar o atleta quando o treino foi salvo para revisão por dados incompletos ou incompatíveis. A versão local passa para `0.23.0` e a novidade foi adicionada à tela de atualizações. A mudança não altera o `rules-v1`, o catálogo, o banco ou a infraestrutura. `go test -count=1 ./...`, `go vet ./...`, `npm run build` e `git diff --check` passaram. A validação manual confirmou `distance_duration_incompatible`, `eligible_for_history: false`, `progression_eligible: false` e `used_for_prescription: false` após concluir um treino com 3 minutos e 55 km. A implementação foi registrada no commit `74f9493`; produção permanece em `0.20.0` até deploy autorizado.

### Continuidade — correção segura de métricas do pedal — 13 de setembro de 2026

Após a classificação de registros inconsistentes, foi implementada uma forma controlada de correção para não obrigar o atleta a conviver com um dado evidentemente errado nem permitir edição ampla do histórico. A rota autenticada `POST /v1/workouts/{workoutID}/correct` aceita somente métricas opcionais do pedal em sessão concluída inelegível: distância, elevação, potência média e frequência cardíaca média. Duração, RPE, feedback, plano ativo e prescrição não podem ser alterados por esse fluxo.

O repositório bloqueia a sessão dentro de uma transação, guarda os valores originais em `workouts.explanation.data_integrity_corrections`, grava os novos valores e recalcula `data-integrity-v1`. A operação é autorizada somente quando a sessão ainda está fora da observação histórica; se a correção continuar incoerente, ela permanece preservada e inelegível. A interface orienta o atleta e informa que o plano não será recalculado. `rules-v1` continua sendo o único motor prescritivo.

A versão local passou para `0.24.0` e a documentação de API foi atualizada. A validação local passou com `go test -count=1 ./...`, `go vet ./...`, `npm run build` e `git diff --check`. No navegador, uma sessão de 3 minutos e 55 km foi corrigida para `0,3 km`; após atualizar a página, o registro permaneceu coerente, o aviso desapareceu, duração/RPE/plano não mudaram e a auditoria PostgreSQL preservou o valor original. A API foi reiniciada para carregar a rota nova. Não houve migração, deploy ou alteração de infraestrutura; a produção permanece em `0.20.0`.

### Feedback pós-treino com contexto estruturado — versão local `0.25.0`
...
### Continuidade — piloto de limiar controlado — 14 de setembro de 2026
