...
Toda atualização com funcionalidade visível deve atualizar `frontend/lib/release.ts`, incrementando `APP_VERSION` e registrando a novidade em `UPDATE_NOTES`, para que ela apareça na tela de novidades após a atualização. A produção oficial está no commit `9d8c624`, com a migração `000016` aplicada, a versão `0.8.0` confirmada na release do GitHub e o piloto XCO publicado após validação e autorização explícita. Não fazer commit, deploy ou mudança de infraestrutura sem autorização explícita.
...
Os testes locais `go test -count=1 ./...`, `go vet ./...`, `npm run build`, lint direcionado dos componentes alterados e `npm audit --omit=dev --audit-level=high` passaram. O lint geral ainda conserva pendências anteriores; o grafo de desenvolvimento mantém avisos sem correção automática disponível. O compose local também passou a fixar o digest do `cloudflared 2026.7.3` ARM64 atualmente executado, sem aplicar essa mudança na VPS. Nenhum commit, deploy ou migração foi realizado. A produção permanece no commit `9d8c624` e na versão `0.8.0`; o commit base local é `a54143b`.
