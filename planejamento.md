...
A funcionalidade visível atualizou a versão local para `0.17.0` e a tela de novidades. A suíte Go, o `go vet`, o build do frontend e a verificação do diff passaram; o lint geral mantém pendências preexistentes fora desta fatia. A migração `000020_completion_context` e o teste SQL ainda precisam ser aplicados no PostgreSQL local, seguidos do teste manual no navegador com uma sessão parcial e um motivo selecionado. Não houve commit, deploy ou alteração de infraestrutura.
