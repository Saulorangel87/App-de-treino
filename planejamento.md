...
Foram adicionados testes para evidência completa, lacuna de carga, esforço alto, períodos inconsistentes e isolamento prescritivo. `go test -count=1 ./...`, `go vet ./...`, `npm run build` e `git diff --check` passaram; o lint geral continua com pendências antigas fora desta mudança. Próxima etapa: revisar a implementação e validar o novo bloco via API local após concluir uma sessão, conferindo `explanation.adaptation_shadow.load_tolerance` sem esperar o feedback real de produção.
