...
Após o commit `810183c`, a quinta fatia começou localmente, sem commit ou deploy. O snapshot de novos rascunhos passa a registrar `rules_v2_shadow`, uma avaliação paralela com versão `rules-v2`, modo `shadow` e escopo `plan_generation_only`; o `engine_version` permanece `rules-v1`.
