...
A migração `000019_short_intervals_evidence` registra as fontes do ensaio de 2025 e do estudo com ciclistas de 2020. A versão local foi atualizada para `0.15.0` e a tela de novidades foi preparada. Foram adicionados testes positivos, de não seleção, de proteção por dor e de validação da preferência; a suíte Go, o `go vet`, o build e a verificação da migração local passaram. No navegador, a opção e a nota aparecem, mas a automação não persistiu a alternância do checkbox controlado pelo React; falta confirmar manualmente a preferência até a geração do plano. O lint geral continua com pendências antigas fora desta fatia. Produção continua em `0.12.0`/`000016`, sem deploy ou alteração de infraestrutura. Após a validação manual, oferecer o commit antes de avaliar a eventual publicação.
