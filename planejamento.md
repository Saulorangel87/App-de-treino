...
Toda atualização com funcionalidade visível deve atualizar `frontend/lib/release.ts`, incrementando `APP_VERSION` e registrando a novidade em `UPDATE_NOTES`, para que ela apareça na tela de novidades após a atualização. A produção oficial está no commit `61d7939`, na versão `0.12.0`, com a release [v0.12.0](https://github.com/Saulorangel87/App-de-treino/releases/tag/v0.12.0) publicada no GitHub. O deploy de 11 de setembro foi validado após backup preventivo, build, manutenção e checagem dos domínios oficiais. Não fazer commit, deploy ou mudança de infraestrutura sem autorização explícita.
