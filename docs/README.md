...
A produção oficial registrada está no commit `f0fec8b`, na VPS Oracle, exposta pelos domínios `cadencia.devsaulo.com.br` e `cadencia-api.devsaulo.com.br`; as migrações até `000021`, o catálogo, os pilotos e as correções técnicas foram publicados após backup, validação e autorização explícita. As auditorias observacionais de periodização, seleção de estímulos e coerência integrada também foram publicadas sem alterar a versão visível `0.20.0`, registrada na release [v0.20.0](https://github.com/Saulorangel87/App-de-treino/releases/tag/v0.20.0). A tela autenticada `/plano` e o JSON de `stimulus_selection_shadow` foram validados na sessão real; os demais snapshots continuam sem representação visual própria. O estado e as lacunas de validação estão em [`project-status.md`](project-status.md).
