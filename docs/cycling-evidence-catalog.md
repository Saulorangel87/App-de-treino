...
A validação local passou pela suíte Go, `go vet`, build do frontend e conferência manual no navegador. O cenário elegível apresentou **Limiar controlado** com RPE 7,5 e três blocos; nível menor, histórico insuficiente, disponibilidade curta e dor impediram a seleção. O piloto foi posteriormente incorporado ao catálogo publicado na versão `0.30.0`; a evidência continua limitada e não autoriza ampliar carga automaticamente.
