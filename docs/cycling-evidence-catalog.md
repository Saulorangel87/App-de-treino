...
**Decisão provisória:** avaliar primeiro uma versão curta e autorregulada, com RPE e controle técnico, em vez de liberar `all-out` para perfis gerais. A especificação deverá definir modalidade explícita, nível e histórico mínimos, avaliação apta, disponibilidade, número máximo de blocos, recuperação, uma única sessão de qualidade, travas por dor/fadiga/recuperação e teste de não seleção. Até essa revisão, o candidato não entra no `rules-v1`, não recebe migração, não altera `APP_VERSION` e não é publicado.

A especificação preliminar foi registrada em `docs/training-adaptation-rules.md`: seis repetições de 1 minuto em RPE 7–8, recuperação leve autorregulada, aquecimento/desaquecimento, nível avançado, estrada ou indoor e gates de segurança. Ela é uma hipótese operacional para teste, não uma conclusão científica nem uma prescrição liberada. A próxima decisão depende da revisão dos gates e da matriz mínima de seleção/não seleção.
