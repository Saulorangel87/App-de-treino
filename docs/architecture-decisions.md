...
A continuidade seguirá o documento canônico `planejamento.md`, com foco exclusivo em ciclismo. Os tópicos de prontidão, regras versionadas, adaptação, progressão/carga, integridade, segurança, feedback e auditabilidade já foram implementados no MVP; a evolução em shadow continua observacional. O `rules-v1` permanece preservado até que uma evolução paralela esteja testada, comparável e auditável. Dados ausentes ou inconsistentes não devem ser tratados como autorização para aumentar carga. A IA continua explicativa e subordinada às regras validadas; não prescreve, inventa evidências ou diagnostica. Corrida e musculação não fazem parte desta decisão e serão consideradas apenas em produtos separados.
...
A confirmação acontece no backend e não depende apenas da interface. O teste transacional `database/tests/account_deletion.sql` cobre a cascata sem deixar dados de teste. Cópias de backup podem permanecer até o prazo de retenção operacional, conforme documentado na tela. A funcionalidade não cria nova migração nem altera o `rules-v1`.
