...
Sexta fatia local, ainda sem commit ou publicação: `rules-v2-adaptation-v1` avalia o feedback pós-treino em modo `shadow`, sem ser chamada pelo trigger transacional atualmente ativo. Ela mantém proteção para dor/esforço alto, transforma uma resposta fácil isolada em `defer_progression` e só registra `progress_duration_5pct` como candidata quando há dois períodos recentes íntegros com carga, feedback e recuperação. Todos os resultados mantêm `progression_eligible`, `applied` e `used_for_prescription` como `false`. A decisão preserva o trigger e o comportamento prescritivo do `rules-v1` enquanto a nova regra é revisada.
