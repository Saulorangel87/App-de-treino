...
WITH athlete_profiles(id, user_id) AS (
    ...
    VALUES
    ('session-past', 'workout-past', 'profile-1', 'completed', now() - interval '1 day', NULL, 40, 5, 20::numeric, 100, 200, 140, now() - interval '1 day'),
    ('session-future', 'workout-future', 'profile-1', 'completed', now() + interval '2 hours', NULL, 120, 10, 80::numeric, 500, 400, 180, now()),
    ('session-cancelled', 'workout-cancelled', 'profile-1', 'cancelled', NULL, now() - interval '2 days', NULL, NULL, NULL, NULL, NULL, NULL, now() - interval '2 days'),
...
if ($summaryActual.Count -ne 1 -or $summaryActual[0] -cne '1|1|40|5|2|20|100|200|140') {
...
if ($weekColumns.Count -ne 9 -or $weekColumns[1..8] -join '|' -cne '1|1|40|5|20|100|200|140') {
