...
### Continuidade — piloto de limiar controlado — versão local `0.28.0` validada
...
A evidência em ciclistas treinados orienta a investigação do formato, mas não valida uma dose universal; por isso, a sessão permanece um piloto conservador, sem potência fixa, sem estimativa automática de limiar, sem progressão automática e sem autoridade adicional para os shadows. Não há migração. `go test -count=1 ./...`, `go vet ./...`, `npm run build` e `git diff --check` passaram; a conferência manual confirmou a seleção elegível e os bloqueios. A funcionalidade foi registrada no commit `d6e36ec`, e o commit `011b204` removeu o cache local indevido do repositório. Produção continua na `0.27.0`.
...
### Continuidade — tratamento correto de falhas nas telas autenticadas — versão local `0.22.0`
