...
No checkout atual, as migrações `000015`, `000016`, `000017`, `000018` e `000019` registram as fontes científicas do catálogo; `000017`, `000018` e `000019` são locais nesta etapa e ainda não foram aplicadas na produção. Na VPS, a produção permanece aplicada até `000016`. O catálogo inicial, o piloto `road_moderate_intervals` e o piloto `xco_aerobic_intervals` foram publicados após revisão, backup verificável, execução ordenada pelo perfil `maintenance`, validação e autorização explícita; a novidade foi registrada em `frontend/lib/release.ts` antes da publicação. O taper pré-prova, o piloto VO₂max de estrada e o piloto de intervalos curtos autorregulados ainda dependem de revisão, backup, autorização e execução controlada antes de qualquer deploy.
