...
const planningTrainingHistoryQuery = `
	...
	performed AS (
		SELECT window_sizes.window_days,
			...
			COUNT(ws.id) FILTER (WHERE f.satisfaction BETWEEN 1 AND 5) AS sessions_with_satisfaction,
			COUNT(ws.id) FILTER (WHERE f.terrain IN ('flat', 'rolling', 'hilly', 'mixed', 'technical', 'indoor')) AS sessions_with_terrain,
			COUNT(ws.id) FILTER (WHERE f.external_conditions IN ('normal', 'heat', 'cold', 'wind', 'rain', 'poor_visibility', 'other')) AS sessions_with_external_conditions,
	...
	SELECT performed.window_days, adherence.expected_sessions, adherence.scheduled_completed_sessions,
		...
		performed.sessions_with_complete_feedback, performed.sessions_with_satisfaction,
		performed.sessions_with_terrain, performed.sessions_with_external_conditions,
		performed.pain_reported_sessions,
...
func (s *Store) trainingHistoryByProfileID(ctx context.Context, profileID string) ([]planning.TrainingHistoryWindow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&window.SessionsWithCompleteFeedback, &window.SessionsWithSatisfaction,
			&window.SessionsWithTerrain, &window.SessionsWithExternalConditions,
...
func trainingHistoryPeriodsFrom(ctx context.Context, queryer rowsQuerier, profileID string) ([]planning.TrainingHistoryPeriod, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&period.SessionsWithCompleteFeedback, &period.SessionsWithSatisfaction,
			&period.SessionsWithTerrain, &period.SessionsWithExternalConditions,
			&period.PainReportedSessions,
		...
		if sessionID != nil && sessionStatus != nil {
			...
			if difficulty != nil && painReported != nil && fatigueAfter != nil {
				workout.Session.Feedback = &planning.Feedback{
					...
					FatigueAfter: *fatigueAfter, RecoveryAfter: recoveryAfter, RepeatConfidence: repeatConfidence, Satisfaction: satisfaction,
				}
				...
				if terrain != nil {
					workout.Session.Feedback.Terrain = *terrain
				}
